...
class Env:
    def __init__(self, ganache: Ganache, project_path: str, config: str | dict = None):
        ...
        Args:
            ...
            config (str|dict): Path to the configuration file or a dictionary containing the configuration. If None, the global configuration will be used.
        Raises:
    ...
    def init(self):
        ...
        if (
            self.start_ganache(
                self.config.get("force_stop", False),
                True,
                Path(self.log_path, "ganache.log"),
            )
            == False
        ):
        ...
        self.run_truffle_migrate(
            self.config["truffle"]["network"]["name"],
            True,
    ...
    def run_truffle_migrate(
        self, network="fuzzing", log_to_file=False, log_file_path=None
    ):
        """
    ...
    def get_accounts(self):
        ...
        """
        return [
            Account(rpc_url=self.rpc_url, **account)
            for account in get_accounts(
                self.rpc_url, self.config["ganache"]["wallet"]["mnemonic"]
            )
        ]
    ...
    def get_deployer_account(self):
        ...
        return Account(
            ...
            **get_accounts(self.rpc_url, self.config["ganache"]["wallet"]["mnemonic"])[
                0
            ],
        )
    ...
    def call_sc_event(self, contract: Contract, event_name: str, tx_hash: str = None):
        return call_sc_event(
            self.rpc_url, contract.abi, contract.address, event_name, tx_hash
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Initialize Truffle/Ganache environment."
    )
    parser.add_argument(
        "--project_path", type=str, help="Path to the Truffle project", required=True
    )
