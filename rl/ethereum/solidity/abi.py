...
class Method:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.inputs = (
            [Argument(**argument) for argument in kwargs["Inputs"]]
            if kwargs["Inputs"] is not None
            else []
        )
        self.outputs = (
            [Argument(**argument) for argument in kwargs["Outputs"]]
            if kwargs["Outputs"] is not None
            else []
        )
        ...
        ]  # 256 is the max num in opcodes list 0xff, the count of the opcode
...
class ABI:
    ...
    def __init__(self, *args, **kwargs):
        ...
        compiled_json_path = os.path.join(
            self.proj_path, "build", "contracts", "{}.json".format(self.contract.name)
        )
