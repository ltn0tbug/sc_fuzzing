...
class Contract:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # check if the contract has the opcodes which can send ether
    ...
    def propagate_state(self):
        """
        propagate the state by simulate the opcodes, each opcode has a state, include stack, memory, storage
        """
        state = EVMState()
        ...
        for insn_idx in range(insn_idx, len(self.insns)):
            ...
            with StackChecker(insn, state):
                if 0x60 <= op <= 0x7f:
                    ...
                    if op == STOP:
                        break
                    # deal with the Numeral Calculations
                    elif op == ADD:
