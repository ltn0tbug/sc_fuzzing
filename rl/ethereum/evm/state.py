...
class EVMState:
    ...
    def mem_reset(self, offset, length):
        """
        reset the self.men[offset:offset+length] to Top()
        """
    ...
    def mem_valid(self, offset, length):
        """
        check men[offset:offset+length] all is not Top
        """
        if len(self.mem) < offset + length:
            return False
        else:
    ...
    def mem_store(self, offset, values):
        """
        stroe values in mem[offset:offset+len(values)]
        """
    ...
    def pop_stack(self, size=1):
        """
        return vals, vals[0] = stack[-1], vals[1] = stack[-2]...
        """
        if size == 1:
            s = self.stack.pop()
            if not is_top(s):
