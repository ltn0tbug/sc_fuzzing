...
class CFG:
    ...
    def init_blocks(self):
        ...
        # initialize the block whose length is None
        for block in self.blocks.values():
    ...
    def add_edge(self, src_idx, tgt_idx, branch_or_default, is_dep):
        """
        branch_or_default: defaule is that src_idx + block.length - 1 == tgt_idx
        is_dep: src_idx has more than one tge_idx(branch) or not
        """
...
class CFGBlock:
    ...
    def add_block_to_method(self, method):
        """
        find all blocks in the method
        """
        method.blocks.add(self.start_idx)

        # count the frequency of opcodes in method
    ...
    def trace_mstore_arg(self, insn_idx, offset, method, depth, visited_blocks):
        """
        deal with the opcode about memory
        """
        if self.start_idx not in visited_blocks:
            visited_blocks[self.start_idx] = 0
        visited_blocks[self.start_idx] += 1
    ...
    def trace_arg(
        ...
        target_op: the op that finds the arg
        """
        if depth > 500:
            return None
        ...
        if is_push(op):
            # when we find the push.arg, we find the actual value
            ...
            if op == POP:
                ...
                # use None to deal with the storage
                if stack_pos == -1 and not is_start:
                    return None
                else:
                    stack_pos += STACK_CHANGES[op]
        ...
        if insn_idx > self.start_idx:
            ...
            # if more than one value, return None
            if len(results) == 1:
                return list(results)[0]
            else:
                return None
