...
class Environment:
    ...
    def __init__(self, limit, seed, max_episode, start_time):
        ...
        self.start_train = 3 * max_episode
    ...
    def MADFuzz_drqn(self, policy, obs, start_time, args):
        ...
        progress_bar = tqdm.tqdm(
            range(1 + init_limit, self.limit + 1),
            desc=f"Fuzzing {args.contract}",
            leave=False,
        )
        for i in progress_bar:
            ...
            if i % self.max_episode < self.max_episode / 10:
                tx, action, new_hiddens, arg_actions = policy.select_tx(
                    state,
                    x_method,
                    contract,
                    obs,
            ...
            for j, action in enumerate(valid_action):
                if len(valid_action[action]) > 0:
                    for method in valid_action[action]:
                        action_cov[j] += method_cov[method]["block_cov"] / len(
                            valid_action[action]
                        )
                else:
                    action_cov[j] = 1
            ...
            if len(arg_actions[1]) > 0:
                policy.uint_agent.store_transition(
                    state, arg_actions[1][0], reward_a, i
                )
            if len(arg_actions[2]) > 0:
                policy.bool_agent.store_transition(
                    state, arg_actions[2][0], reward_a, i
                )
            if len(arg_actions[3]) > 0:
                policy.addr_agent.store_transition(
                    state, arg_actions[3][0], reward_a, i
                )
            if len(arg_actions[4]) > 0:
                policy.byte_agent.store_transition(
                    state, arg_actions[4][0], reward_a, i
                )
            state = next_state
            ...
            # if i % self.max_episode == 0:
            #     print(f'elapsed_time: {round(elapsed_time,2)}, iteration: {i}, reward_f: {reward_f}, reward_a: {reward_a}')
            # # LOG.info(obs.stat)
            progress_bar.set_postfix(
                block_coverage=f"{round(new_block_coverage,2)}",
                insn_coverage=f"{round(new_insn_coverage,2)}",
                reward_f=reward_f,
                reward_a=reward_a,
            )
            ...
            if i % self.max_episode == 0 and elapsed_time < self.limit:
                ...
                if i >= self.start_train:
                    if args.mode == "train":
                        ...
                        episole = init_episole - 0.6 * (i - self.start_train) / (
                            self.limit - self.start_train
                        )
        ...
        LOG.info(obs.stat)
        # with open(f'{args.output_path}/{args.address}/drqn_result.json', 'w') as f:
    ...
    def fuzz_loop_RL(self, policy, obs, start_time, args):
        ...
        for i in range(1 + init_limit, self.limit + 1):
            ...
            if i % self.max_episode < self.max_episode / 10:
                tx, action, hidden, arg_actions = policy.select_tx(
                    state,
                    x_method,
                    contract,
                    obs,
            ...
            if i % self.max_episode == 0:
                ...
                action_cov = action_cov.mean()
                # print(action_cov)
                new_insn_coverage, new_block_coverage = obs.stat.get_coverage(
                    tx.contract
                )
    ...
    def init_txs_RL(self, policy, obs, limit, result):
        ...
        """
        policy_frandom = PolicyFRandom(
            policy.execution, policy.contract_manager, policy.account_manager
        )
        result["init_txs_RL"] = []
    ...
    def fuzz_loop(self, policy, obs):
        ...
        for i in range(1, self.limit + 1):
            if (
                policy.__class__ in (PolicyRandom, PolicyFRandom)
                and i > self.limit // 2
            ):
                # reset the balance of the contracts
                for contract_name in policy.contract_manager.fuzz_contract_names:
