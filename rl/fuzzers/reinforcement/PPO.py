...
class ActorCritic(nn.Module):
    ...
    def set_action_std(self, new_action_std):
        if self.has_continuous_action_space:
            self.action_var = torch.full((self.action_dim,), new_action_std**2).to(
                device
            )
        else:
    ...
    def evaluate(self, state, action):
        if self.has_continuous_action_space:
            ...
            dist = MultivariateNormal(action_mean, cov_mat)
            action = (
                action.reshape(-1, self.action_dim) if self.action_dim == 1 else action
            )
        else:
            action_probs = self.actor(state)
            dist = Categorical(action_probs)
...
class PPO:
    ...
    def select_action(self, state):
        ...
        return (
            action.cpu().numpy() if self.has_continuous_action_space else action.item()
        )
    ...
    def update(self):
        ...
        for reward, is_terminal in zip(
            reversed(rewards), reversed(self.buffer.is_terminals)
        ):
            if is_terminal:
                discounted_reward = 0
        ...
        # Collect buffer data
        states = (
            torch.stack(self.buffer.states[: len(discounted_rewards)])
            .detach()
            .to(device)
        )
        actions = (
            torch.stack(self.buffer.actions[: len(discounted_rewards)])
            .detach()
            .to(device)
        )
        old_logprobs = (
            torch.stack(self.buffer.logprobs[: len(discounted_rewards)])
            .detach()
            .to(device)
        )
        ...
        state_values = (
            torch.stack(self.buffer.state_values[: len(discounted_rewards)])
            .detach()
            .to(device)
        )
        ...
        for _ in range(self.K_epochs):
            ...
            logprobs, current_state_values, dist_entropy = self.policy.evaluate(
                states, actions
            )
            ...
            loss = (
                ...
                + 0.5 * self.MseLoss(current_state_values, discounted_rewards)
                - 0.01 * dist_entropy
            )
    ...
    def load(self, checkpoint_path):
        if "drqn_eval_net.pth" in os.listdir(f"{checkpoint_path}"):
            self.policy_old.load_state_dict(
                torch.load(f"{checkpoint_path}/ppo_eval_net.pth", map_location=device)
            )
