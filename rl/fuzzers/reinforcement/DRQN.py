...
class DRQN:
    ...
    def choose_action(
        ...
        hidden=None,
        episole=0.2,
        agent_action_count_array=np.zeros(7),
    ):
        ...
        if hidden == None:
            hidden = (
                Variable(
                    torch.zeros(1, 1, self.eval_net.lstm_i_dim).float().to(device)
                ),
                Variable(
                    torch.zeros(1, 1, self.eval_net.lstm_i_dim).float().to(device)
                ),
    ...
    def load(self, model_path):
        ...
        if "drqn_eval_net.pth" in os.listdir(f"{model_path}"):
            if torch.cuda.is_available():
                print("load from cuda")
                self.eval_net.load_state_dict(
                    torch.load(f"{model_path}/drqn_eval_net.pth")
                )
            else:
                print("load from cpu")
                self.eval_net.load_state_dict(
                    torch.load(
                        f"{model_path}/drqn_eval_net.pth",
                        map_location=torch.device("cpu"),
                    )
                )
            print("load model from {}".format(f"{model_path}/drqn_eval_net.pth"))
            print("====================================")
