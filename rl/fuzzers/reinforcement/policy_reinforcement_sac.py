...
class PolicyReinforcementSAC(PolicyBase):
    ...
    def load_model(self):
        ...
        if use_cuda == "cuda":
            self.compress_net.load_state_dict(
                torch.load(os.path.join(load_dir, "net.pt"))
            )
        else:
            self.compress_net.load_state_dict(
                torch.load(os.path.join(load_dir, "net.pt"), map_location="cpu")
            )
        ...
        if contract.name not in self.last_method:
            ...
            with torch.no_grad():
                last_method_feature = self.calc_method_features(
                    contract.name, method_feats, True
                )
    ...
    def _select_arguments(
        self, contract, method, sender, obs, x_state, x_method, hiddens, episole
    ):
        ...
        for arg in method.inputs:
            ...
            if t == SolType.IntTy or t == SolType.UintTy:
                if t == SolType.IntTy:
                    value, new_hidden_1 = self._select_int(
                        contract,
                        method,
                        arg.evm_type.size,
                        obs,
                    ...
                    value, new_hidden_2 = self._select_uint(
                        contract,
                        method,
                        arg.evm_type.size,
                        obs,
                ...
                arg, arr_hiddens = self._select_slice(
                    contract,
                    method,
                    sender,
                    arg.evm_type.elem,
                ...
                arg, arr_hiddens = self._select_array(
                    contract,
                    method,
                    sender,
                    arg.evm_type.size,
                ...
                # TODO select address
                value, new_hidden_4 = self._select_address(
                    sender, x_state, hiddens[4], episole
                )
                arguments.append(value)
            elif t == SolType.FixedBytesTy:
                value, new_hidden_5 = self._select_fixed_bytes(
                    arg.evm_type.size, obs, x_state, hiddens[5], episole
                )
                arguments.append(value)
            elif t == SolType.BytesTy:
                value, new_hidden_5 = self._select_bytes(
                    obs, x_state, hiddens[5], episole
                )
                arguments.append(value)
            else:
    ...
    def _select_int(
        self, contract, method, size, obs, chosen_int, x_state, hidden, episole
    ):
        valid_action_indices = list(range(0, size - 1))
        action = self.int_agent.choose_action(x_state)

        value = int(
            map_to_range(action, new_min=-(1 << size - 1), new_max=(1 << size - 1) - 1)
        )
    ...
    def _select_array(
        self, contract, method, sender, size, typ, obs, x_state, hiddens, episole
    ):
        t = typ.t
        arr = []
        ...
        for _ in range(size):
            ...
            if t == SolType.IntTy or t == SolType.UintTy:
                ...
                arg, arr_hiddens = self._select_slice(
                    contract, method, sender, typ.elem, obs, x_state, hiddens, episole
                )
                arr.append(arg)
            elif t == SolType.ArrayTy:
                arg, arr_hiddens = self._select_array(
                ...
                value, new_hidden_5 = self._select_fixed_bytes(
                    typ.size, obs, x_state, hiddens[5], episole
                )
                arr.append(value)
            elif t == SolType.BytesTy:
                value, new_hidden_5 = self._select_bytes(
