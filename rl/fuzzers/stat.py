...
class Stat:
    ...
    def update(self, logger):
        ...
        """
        # print(dir(logger))
        # print(logger.trace_log_storage)
        destruct = False
        ...
        for i, log in enumerate(logger.logs):
            ...
            if log.pc in cur_contract.insn_pc_to_idx:
                ...
                if insn_idx in cur_contract.cfg.insn_idx_to_block:
                    ...
                    self.temp_covered_blocks_dict[cur_contract.name].add(
                        block.start_idx
                    )  # temp

        if (
            logger.tx.method
            not in self.contract_manager[logger.tx.contract].abi.methods_by_name
        ):
            return 0, {}, {}
        ...
        for checker in self.checkers:
            if checker.check(logger):
                ...
                self.bug_res[logger.tx.contract][checker.__class__.__name__].add(
                    logger.tx.method
                )
    ...
    def to_json(self):
        ...
        for name in self.contract_manager.fuzz_contract_names:
            ...
            coverage_insns += (
                covered_insns / all_insns
            )  # the proportion of the coverage_insns
        ...
        contract_jsons["insn_coverage"] = coverage_insns / len(
            self.contract_manager.fuzz_contract_names
        )  # the average of the insn_coverage
        contract_jsons["block_coverage"] = coverage_blocks / len(
            self.contract_manager.fuzz_contract_names
        )  # the average of the blocK_coverage
        ...
        for name in self.contract_manager.fuzz_contract_names:
            ...
            contract_json["temp_insn_coverage"] = (
                len(self.temp_covered_pcs_dict[name]) / all_insns
            )
            ...
            contract_json["temp_block_coverage"] = (
                len(self.temp_covered_blocks_dict[name]) / all_blocks
            )
