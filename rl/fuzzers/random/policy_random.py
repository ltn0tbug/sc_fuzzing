...
class PolicyRandom(PolicyBase):
    ...
    def select_tx_for_method(self, contract, method, obs):
        ...
        arguments = self._select_arguments(method, sender)
    ...
    def select_tx_with_method(self, contract, method, obs):
        sender = self._select_sender()
        arguments = self._select_arguments(method, sender)
        amount = self._select_amount(contract, method, sender, obs)
        timestamp = self._select_timestamp(obs)
        return sender, arguments, amount, timestamp
    ...
    def _select_arguments(self, method, sender):
        arguments = []
        for arg in method.inputs:
            solidity_type = arg.evm_type
            arguments.append(self._select_value(solidity_type, sender))
            # t = arg.evm_type.t
            # if t == SolType.IntTy:
    ...
    def _select_array(self, typ, sender, size=None):
        if size is None:
            size = random.randint(1, 15)
        return [self._select_value(typ, sender) for _ in range(size)]
    ...
    def _select_bytes(self, size=None):
        if size is None:
