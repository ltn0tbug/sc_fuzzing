...
class RecordManager:
        ...
        for contract in self.contract_manager.contract_dict.values():
            ...
            for method in contract.abi.methods:
                self.method_records[contract.name][method.name] = MethodRecord(
                    ...
                    (
                        0
                        if len(contract.insns) == 0
                        else (len(method.insns) / len(contract.insns))
                    ),
                    (
                        0
                        if len(contract.cfg.blocks) == 0
                        else (len(method.blocks) / len(contract.cfg.blocks))
                    ),
                )
    ...
    def get_method_coverage(self, contract):
        ...
        for method_name, method_record in self.method_records[contract].items():
            ...
            res[method_name]["insn_cov"] = (
                len(method_record.covered_insns) / method_record.len_insns
                if method_record.len_insns > 0
                else 0
            )
            res[method_name]["block_cov"] = (
                len(method_record.covered_blocks) / method_record.len_blocks
                if method_record.len_blocks > 0
                else 0
            )
        return res
    ...
    def update(self, logger, insn_coverage_change, block_coverage_change):
        ...
        for i, log in enumerate(logger.logs):
            ...
            if (
                cur_contract.name == tx.contract
                and log.pc in cur_contract.insn_pc_to_idx
            ):
...
class MethodRecord:
    ...
    def to_vec(self, tx_count):
        ...
        feature.append(
            self.tx_count / tx_count if tx_count else 0
        )  # Fraction of transactions in txs that calls self method
        feature.append(
            len(self.covered_insns) / self.len_insns if self.len_insns > 0 else 0
        )  # Instruction coverage of self method
        ...
        feature.append(
            len(self.covered_blocks) / self.len_blocks if self.len_blocks > 0 else 0
        )  # Block coverage of self method
        ...
        feature.append(
            (self.count_revert / self.tx_count) if self.tx_count else 0
        )  # Fraction of transactions in txs that end with revert
        feature.append(
            (self.count_invalid / self.tx_count) if self.tx_count else 0
        )  # Fraction of transactions in txs that end with invalid
        feature.append(
            (self.count_return / self.tx_count) if self.tx_count else 0
        )  # Fraction of transactions in txs that end with return
