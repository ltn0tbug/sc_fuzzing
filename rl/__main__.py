...
from .fuzzers.reinforcement import (
...
def get_args():
    ...
    parser.add_argument(
        "--execution", dest="execution", type=str, default="./execution.so"
    )  # the Dynamic library
    parser.add_argument(
        "--proj", dest="proj", type=str, default=None
    )  # the backend of fuzz
    parser.add_argument(
        "--contract", dest="contract", type=str, default=None
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "frandom",
            "reinforcement_drqn",
            "reinforcement_ppo_discrete",
            "reinforcement_ppo_continuous",
            "reinforcement_sac",
        ],
        default="random",
    )  # the mode of fuzzer symbolic=training

    parser.add_argument(
        "--model", dest="model", type=str, default="model_imitation"
    )  # the model used to fuzz
    ...
    parser.add_argument(
        "--train_dir", dest="train_dir", type=str, default=None
    )  # the train data used to fuzz
    parser.add_argument(
        "--dataset_dump_path", dest="dataset_dump_path", type=str, default=None
    )  # the export path of train data
    ...
    parser.add_argument(
        "--reward", dest="reward", choices=["cov", "cov+bugs", "bugs"], default=None
    )
    ...
    parser.add_argument(
        "--detect_bugs", dest="detect_bugs", choices=["all", "ls"], default=None
    )
    parser.add_argument("--limit_time", dest="limit_time", type=int, default=1800)

    parser.add_argument(
        "--dvc", type=str, default="cpu", help="running device: cuda or cpu"
    )
    parser.add_argument(
        "--EnvIdex",
        type=int,
    ...
    parser.add_argument(
        "--Max_train_steps", type=int, default=int(5e6), help="Max training steps"
    )
    parser.add_argument(
        "--save_interval",
        type=int,
    ...
    parser.add_argument(
        "--update_every", type=int, default=50, help="Training Fraquency, in stpes"
    )
    ...
    parser.add_argument(
        "--a_lr", type=float, default=3e-4, help="Learning rate of actor"
    )
    parser.add_argument(
        "--c_lr", type=float, default=3e-4, help="Learning rate of critic"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Use adaptive_alpha or Not",
    )
...
def main():
    ...
    if args.fuzzer == "random":
        ...
        policy = PolicyReinforcementDRQN(
            execution, contract_manager, account_manager, args
        )
        # input('stop')
        if args.mode == "train":  # args.train_dir is not None:
            ...
            print("train mode")
        policy.load_model()
        obs = ObsReinforcement(
            contract_manager, account_manager, args.dataset_dump_path
        )
    elif args.fuzzer == "reinforcement_ppo_discrete":
        policy = PolicyReinforcementPPODiscrete(
            execution, contract_manager, account_manager, args
        )
        # input('stop')
        if args.mode == "train":  # args.train_dir is not None:
        ...
        policy = PolicyReinforcementPPOContinuous(
            execution, contract_manager, account_manager, args
        )
        # input('stop')
        if args.mode == "train":
