...
OP_NAME_REVERSED = {v: k for k, v in OP_NAME.items()}


class Log:
    """
    one Log mean one opcode
    """

    def __init__(self, *args, **kwargs):
        self.pc = kwargs["pc"]
        self.op = OP_NAME_REVERSED[kwargs["op"]]
        self.gas = kwargs["gas"]
        self.gas_cost = kwargs["gasCost"]
        self.memory = kwargs["memory"]
        self.memory_size = len(kwargs["memory"]) * 32
        self.stack = kwargs["stack"]
        self.depth = kwargs["depth"]
        self.op_name = kwargs["op"]
...
class Logger:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logs = (
            [Log(**log) for log in kwargs["logs"]]
            if kwargs["logs"] is not None
            else None
        )
                        ...
                        for j in range(i, -1, -1):
                            if self.logs[j].depth < log.depth:
                                ...
                                if self.logs[j].op in (CALL, CALLCODE):
                                    ...
                                    v0, v1 = self.trace_log_memory(
                                        j - 1, args_offset, args_offset + args_length
                                    )
                                    value_from_call |= v0
                                    value_from_block |= v1
                                    break
    ...
    def trace_log_storage(self, log_idx, offset):
        for i in range(log_idx, -1, -1):
            log = self.logs[i]
    ...
    def trace_log_stack(self, log_idx, stack_pos):
        ...
        if 0x60 <= op <= 0x7F:
            ...
            if op == STOP:
                ...
                if stack_pos == -1:
                    value_from_call0, value_from_block0 = self.trace_log_stack(
                        log_idx - 1, -1
                    )
                    value_from_call1, value_from_block1 = self.trace_log_stack(
                        log_idx - 1, -2
                    )
                    ...
                    return self.trace_log_stack(
                        log_idx - 1, stack_pos + STACK_CHANGES[op]
                    )
                ...
                if stack_pos == -1:
                    ...
                    value_from_call2, value_from_block2 = self.trace_log_stack(
                        log_idx - 1, -3
                    )
                    value_from_call = (
                        value_from_call0 | value_from_call1 | value_from_call2
                    )
                    value_from_block = (
                        value_from_block0 | value_from_block1 | value_from_block2
                    )
                    return value_from_call, value_from_block
                else:
                    return self.trace_log_stack(
                ...
                if stack_pos == -1:
                    value_from_call, value_from_block = self.trace_log_stack(
                        log_idx - 1, -1
                    )
                    return value_from_call, value_from_block
                else:
            ...
            if op == SHA3:
                if stack_pos == -1:
                    low, high = int(stack[-1], 16), int(stack[-1], 16) + int(
                        stack[-2], 16
                    )
            ...
            if op == POP:
                ...
                if stack_pos == -1:
                    ...
                    value_from_call1, value_from_block1 = self.trace_log_storage(
                        log_idx - 1, int(stack[-1], 16)
                    )
                    value_from_call = value_from_call0 | value_from_call1
                    value_from_block = value_from_block0 | value_from_block1
                    return value_from_call, value_from_block
            ...
            if stack_pos == -1:
                ...
                value_from_call1, value_from_block1 = self.trace_log_memory(
                    log_idx - 1, offset, offset + length
                )
                value_from_call = value_from_call0 | value_from_call1
                value_from_block = value_from_block0 | value_from_block1
                return value_from_call, value_from_block
            ...
            if stack_pos == -1:
                ...
                value_from_call3, value_from_block3 = self.trace_log_memory(
                    log_idx - 1, args_offset, args_offset + args_length
                )
                value_from_call = (
                    value_from_call0
                    | value_from_call1
            ...
            value_from_call2, value_from_block2 = self.trace_log_memory(
                log_idx - 1, args_offset, args_offset + args_length
            )
